...
router.get("/api/favorites", authorizeUser, async (req,res) => {
    ...
    console.log(req.session.user.name, sqlData)
...
router.post("/api/favorites", authorizeUser , async (req,res) => {
    const bookResult = await db.run(`INSERT INTO favorites(username, book_id) VALUES (?, ?);`,[req.session.user.name, req.body.book_id]) 
    res.send({data: null });
})


router.delete("/api/favorites:id", authorizeUser , async (req,res) => {
const deleteBookResult = await db.run('DELETE FROM favorites WHERE book_id = ? AND username = ?;',
